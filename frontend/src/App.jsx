...
/* ---------------------------------------------------------------
   RESPONSIVE HELPER — the layouts below use inline-style CSS grid
   (kept simple, no CSS-in-JS dependency); this hook lets the handful
   of multi-column grids collapse to one column on narrow viewports
   instead of shipping a parallel stylesheet for a handful of rules.
------------------------------------------------------------------ */
...
function ResultCard({ result, sourceText, onDownload }) {
  ...
  return (
    <Panel style={{ marginTop: 18 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 20, marginTop: 22 }}>
        <div>
...
/* ---------------------------------------------------------------
   ERROR BOUNDARY — a crash inside one tab (a bad API response shape,
   a chart edge case, etc.) shouldn't blank the whole app. Falls back
   to an inline message with a retry button, scoped to whichever tab
   threw, so navigation and login state are untouched.
------------------------------------------------------------------ */
      ...
      return (
        <Panel>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
            ...
            <span style={{ fontFamily: FONT_DISPLAY, fontWeight: 700, fontSize: 15 }}>This tab hit an unexpected error</span>
          </div>
          <div style={{ fontSize: 13, color: C.sub, marginBottom: 14 }}>
            The rest of the app is unaffected — try again, or switch tabs.
          </div>
          <button
            onClick={() => this.setState({ hasError: false })}
            style={{
              background: C.cyan, color: "#03181C", border: "none", borderRadius: 8,
              padding: "8px 16px", fontWeight: 700, cursor: "pointer", fontFamily: FONT_DISPLAY,
            }}
          >
            RETRY
          </button>
