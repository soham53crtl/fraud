...
#   3. Seed the database with demo accounts + fraud graph + geo complaints
...
# --- 1. Free port 5000 if something is already listening on it ---
...
Write-Host "`n[3/4] Installing frontend dependencies (includes leaflet for the map)..." -ForegroundColor Yellow
...
# --- 4. Seed the database ---
Write-Host "`n[4/4] Seeding demo data into MongoDB..." -ForegroundColor Yellow
Push-Location backend
npm run seed
Pop-Location
...
Write-Host "  Terminal 2:  cd frontend; npm run dev   -> open http://localhost:5173/"
Write-Host "`nDemo logins: citizen@demo.com / citizen123, officer@demo.com / officer123 (etc. — see backend/seed.js)"
