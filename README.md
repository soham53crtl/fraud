...
npm run test                # 9 automated tests (Node's built-in test runner)
npm run dev
...
- **`backend/`** — Express + MongoDB API. JWT auth with role-based access
  ...
  rate limiting on login and AI-analysis endpoints, a real trained NLP model,
  and routes for reports / AI analysis / fraud graph / analytics. See
...
9 tests covering the Naive Bayes classifier and the blended risk engine
(category accuracy, score bounds, model metadata for auditability).
